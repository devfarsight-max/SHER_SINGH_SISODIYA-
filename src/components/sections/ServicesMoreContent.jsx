...
export default function ServicesMoreContent() {
  return <>
    <section className="bg-[#edf8df] px-5 py-24 sm:px-8 lg:py-32">
      <div className="mx-auto max-w-7xl"><Reveal className="max-w-3xl"><p className="text-xs font-bold uppercase tracking-[.2em] text-[#4d7b32]">Service coverage</p><h2 className="mt-4 text-3xl font-semibold tracking-tight sm:text-5xl">Support across the complete supply conversation.</h2><p className="mt-5 leading-8 text-[#526b5d]">Our service begins before the first supply movement. Clear requirement assessment and coordination help create an arrangement that works for both parties.</p></Reveal><div className="mt-12 grid gap-5 md:grid-cols-2">{coverage.map(([title, text], index) => <MotionCard key={title} delay={index * .08} className="rounded-3xl bg-white p-8 shadow-sm"><div className="flex items-start gap-5"><span className="grid size-11 shrink-0 place-items-center rounded-full bg-[#d7f1b1] text-sm font-bold">0{index + 1}</span><div><h3 className="text-xl font-semibold">{title}</h3><p className="mt-3 leading-7 text-[#526b5d]">{text}</p></div></div></MotionCard>)}</div></div>
    </section>
    ...
    <section className="bg-white px-5 py-24 sm:px-8 lg:py-32">
      <div className="mx-auto grid max-w-7xl gap-14 lg:grid-cols-2 lg:items-center lg:gap-20">
        <Reveal><img src="/images/services-hospitality.png" alt="Commercial raw milk delivery for a hospitality business" loading="lazy" className="aspect-[4/3] w-full rounded-[2rem] object-cover shadow-xl" /></Reveal>
        <div><Reveal><p className="text-xs font-bold uppercase tracking-[.2em] text-[#4d7b32]">Buyer-specific needs</p><h2 className="mt-4 text-3xl font-semibold tracking-tight sm:text-4xl">Different businesses. Different operating priorities.</h2><p className="mt-5 leading-8 text-[#526b5d]">The supply discussion is shaped around how each buyer uses raw milk and how important timing, volume, and continuity are to their operation.</p></Reveal><div className="mt-8 space-y-4">{[['Processors','Production planning may require recurring volumes and consistent procurement coordination.'],['Wholesalers and retailers','Regional trade networks benefit from responsive supply planning and market understanding.'],['Hotels and restaurants','Kitchen operations require freshness, practical receiving times, and direct communication.'],['Institutions','Structured requirements may involve larger quantities and defined procurement schedules.']].map(([title, text], index) => <Reveal key={title} delay={index * .07} className="border-b border-[#d3e6c5] pb-4"><h3 className="font-semibold">{title}</h3><p className="mt-2 text-sm leading-6 text-[#526b5d]">{text}</p></Reveal>)}</div></div>
      </div>
    </section>

    <section className="bg-[#113526] px-5 py-24 text-white sm:px-8 lg:py-32">
      <div className="mx-auto max-w-7xl"><Reveal className="grid gap-8 lg:grid-cols-2 lg:items-end"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-[#c7e7a1]">Getting started</p><h2 className="mt-4 text-3xl font-semibold tracking-tight sm:text-5xl">A straightforward business onboarding process.</h2></div><p className="max-w-xl leading-8 text-white/65">Direct discussion helps establish whether the requirement and current supply capability are a suitable match.</p></Reveal><div className="mt-14 grid gap-4 md:grid-cols-2 lg:grid-cols-4">{onboarding.map(([title, text], index) => <MotionCard key={title} delay={index * .08} className="rounded-3xl border border-white/15 bg-white/5 p-7"><span className="text-sm font-bold text-[#c7e7a1]">STEP 0{index + 1}</span><h3 className="mt-8 text-xl font-semibold">{title}</h3><p className="mt-3 text-sm leading-7 text-white/60">{text}</p></MotionCard>)}</div><Reveal className="mt-10"><Link to="/contact" className="btn-primary bg-[#d7f1b1] text-[#113526] hover:bg-white">Start a supply discussion →</Link></Reveal></div>
    </section>

    <section className="px-5 py-24 sm:px-8 lg:py-32">
      <div className="mx-auto grid max-w-7xl gap-14 lg:grid-cols-[.85fr_1.15fr] lg:gap-24"><Reveal><p className="text-xs font-bold uppercase tracking-[.2em] text-[#4d7b32]">Service expectations</p><h2 className="mt-4 text-3xl font-semibold tracking-tight sm:text-4xl">A transparent basis for long-term coordination.</h2><p className="mt-5 leading-8 text-[#526b5d]">A dependable relationship requires both sides to communicate clearly about demand, availability, timing, and operational changes.</p></Reveal><div className="grid gap-5 sm:grid-cols-2">{[['Clear information','Accurate requirement details help support better procurement planning.'],['Realistic commitments','Supply arrangements should reflect available capacity and practical delivery conditions.'],['Timely updates','Early communication helps both sides respond to changes effectively.'],['Ongoing review','Requirements can be reviewed as buyer demand and network capacity evolve.']].map(([title, text], index) => <MotionCard key={title} delay={index * .08} className="rounded-2xl border border-[#d3e6c5] bg-white p-7"><span className="text-xl text-[#659b43]">◆</span><h3 className="mt-6 text-lg font-semibold">{title}</h3><p className="mt-3 text-sm leading-7 text-[#526b5d]">{text}</p></MotionCard>)}</div></div>
    </section>
  </>
}
